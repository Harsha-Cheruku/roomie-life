...
export const PushNotificationPrompt = ({ onDismiss, variant = 'banner' }: PushNotificationPromptProps) => {
  ...
  return (
    <div
      className={cn(
        "fixed top-16 left-2 right-2 z-40 animate-slide-down",
        "bg-card border border-border backdrop-blur-lg",
        "rounded-2xl shadow-lg p-2.5"
      )}
    ...
    >
      <div className="flex items-center gap-2 min-w-0">
        ...
        <button
          onClick={handleDismiss}
          aria-label="Dismiss"
          className="w-8 h-8 -mr-1 rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted shrink-0"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
